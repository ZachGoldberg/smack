...
public class PacketParserUtils {
	private static final Logger LOGGER = Logger
			.getLogger(PacketParserUtils.class.getName());

	public static XmlPullParser getParserFor(String stanza)
		...
		while (event != XmlPullParser.START_TAG) {
			if (event == XmlPullParser.END_DOCUMENT) {
				throw new IllegalArgumentException(
						"Document contains no start tag");
			}
			event = parser.next();
		}
		return parser;
	...
	public static XmlPullParser getParserFor(String stanza, String startTag)
		...
		while (true) {
			...
			if (event == XmlPullParser.START_TAG && name.equals(startTag)) {
				...
				throw new IllegalArgumentException("Could not find start tag '"
						+ startTag + "' in stanza: " + stanza);
			}
			parser.next();
		}
	 ...
	 * 
	 * connection is optional and is used to return feature-not-implemented
	 * errors for unknown IQ stanzas.
	 *
	 * @param parser
	 * @param connection
	 ...
	 * @throws Exception
	 */
	public static Packet parseStanza(XmlPullParser parser,
	...
	/**
	 * Creates a new XmlPullParser suitable for parsing XMPP. This means in
	 * particular that FEATURE_PROCESS_NAMESPACES is enabled.
	 * <p>
	 * Note that not all XmlPullParser implementations will return a String on
	 * <code>getText()</code> if the parser is on START_TAG or END_TAG. So you
	 * must not rely on this behavior when using the parser.
	 * </p>
	 * 
	 ...
	 * contain Mixed Content (as defined in XML 3.2.2), or else an
	 * XmlPullParserException will be thrown.
	 * </p>
	 * This method is used for the parts where the XMPP specification requires
	 * elements that contain only text or are the empty element.
	 * 
	 * @param parser
	 ...
	 * @throws IOException
	 */
	public static String parseElementText(XmlPullParser parser)
			throws XmlPullParserException, IOException {
	 ...
	 * </p>
	 * Note that only the outermost namespace attributes ("xmlns") will be
	 * returned, not nested ones.
	 *
	 * @param parser
	 *            the XML pull parser
	 ...
	 * <p>
	 * The parser must be positioned on the START_TAG of the element which
	 * content is going to get returned. If the current element is the empty
	 * element, then the empty string is returned. If it is a element which
	 * contains just text, then just the text is returned. If it contains nested
	 * elements (and text), then everything from the current opening tag to the
	 * corresponding closing tag of the same depth is returned as String.
	 * </p>
	 * Note that only the outermost namespace attributes ("xmlns") will be
	 * returned, not nested ones.
	 * 
	...
	public static CharSequence parseContent(XmlPullParser parser)
		...
		// Advance the parser, since we want to parse the content of the current
		// element
		parser.next();
		return parseContentDepth(parser, parser.getDepth());
	}
	...
	/**
	 * Returns the content from the current position of the parser up to the
	 * closing tag of the given depth. Note that only the outermost namespace
	 * attributes ("xmlns") will be returned, not nested ones.
	 * <p>
	 * This method is able to parse the content with MX- and KXmlParser. In
	 * order to achieve this some trade-off has to be make, because KXmlParser
	 * does not support xml-roundtrip (ie. return a String on getText() on
	 * START_TAG and END_TAG). We are therefore required to work around this
	 * limitation, which results in only partial support for XML namespaces
	 * ("xmlns"): Only the outermost namespace of elements will be included in
	...
	public static CharSequence parseContentDepth(XmlPullParser parser, int depth)
		...
		// XmlPullParser reports namespaces in nested elements even if *only*
		// the outer ones defines
		// it. This 'flag' ensures that when a namespace is set for an element,
		// it won't be set again
		// in a nested element. It's an ugly workaround that has the potential
		// to break things.
		...
		while (true) {
			if (event == XmlPullParser.START_TAG) {
				...
				for (int i = 0; i < parser.getAttributeCount(); i++) {
					xml.attribute(parser.getAttributeName(i),
				...
				if (isEmptyElement) {
					// Do nothing as the element was already closed, just reset
					// the flag
				...
				if (namespaceElement != null
						&& namespaceElement.equals(parser.getName())) {
					// We are on the closing tag, which defined the namespace as
					// starting tag, reset the 'flag'
		...
		while (!done) {
			...
			if (eventType == XmlPullParser.START_TAG) {
				...
				// Otherwise, it must be a packet extension.
					...
					// Be extra robust: Skip PacketExtensions that cause
					// Exceptions, instead of
					// failing completely here. See SMACK-390 for more
					// information.
					try {
	 ...
	 * @param connection
	 *            the optional XMPPConnection used to send
	 *            feature-not-implemented replies.
	 * @return an IQ object.
	 * @throws Exception
	 *             if an exception occurs while parsing the packet.
	 */
	public static IQ parseIQ(XmlPullParser parser, XMPPConnection connection)
			throws Exception {
		IQ iqPacket = null;
		...
		while (!done) {
			...
			if (eventType == XmlPullParser.START_TAG) {
				...
				// this element name and namespace.
					...
					Object provider = ProviderManager.getIQProvider(
		...
		if (iqPacket == null) {
			if (connection != null
					&& (IQ.Type.get == type || IQ.Type.set == type)) {
				// If the IQ stanza is of type "get" or "set" containing a child
				// element qualified
				// by a namespace with no registered Smack provider, then answer
				// an IQ of type
	 ...
	 *             if an exception occurs while parsing the packet.
	 */
	public static SASLFailure parseSASLFailure(XmlPullParser parser)
			throws Exception {
	...
	public static XMPPError parseError(XmlPullParser parser) throws Exception {
		...
		try {
			...
			LOGGER.log(
					Level.SEVERE,
					"Could not find error type for "
							+ type.toUpperCase(Locale.US), iae);
		}
	...
	public static PacketExtension parsePacketExtension(String elementName,
		...
		Object provider = ProviderManager.getExtensionProvider(elementName,
				namespace);
		if (provider != null) {
	...
	public static Object parseWithIntrospection(String elementName,
			Class<?> objectClass, XmlPullParser parser)
	 ...
	 */
	private static Object decode(Class<?> type, String value)
			throws ClassNotFoundException {
