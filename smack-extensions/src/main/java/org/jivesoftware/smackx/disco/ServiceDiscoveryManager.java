...
public class ServiceDiscoveryManager extends Manager {
	...
	private static final Logger LOGGER = Logger
			.getLogger(ServiceDiscoveryManager.class.getName());
	private static final String DEFAULT_IDENTITY_NAME = "Smack";
	private static final String DEFAULT_IDENTITY_CATEGORY = "client";
	private static final String DEFAULT_IDENTITY_TYPE = "pc";
	private static DiscoverInfo.Identity defaultIdentity = new Identity(
			DEFAULT_IDENTITY_CATEGORY, DEFAULT_IDENTITY_NAME,
	...
	private static Map<XMPPConnection, ServiceDiscoveryManager> instances = Collections
			.synchronizedMap(new WeakHashMap<XMPPConnection, ServiceDiscoveryManager>());
	...
	/**
	 * Set the default identity all new connections will have. If unchanged the
	 * default identity is an identity where category is set to 'client', type
	 * is set to 'pc' and name is set to 'Smack'.
	 * 
	 * @param identity
	 */
	...
	/**
	 * Creates a new ServiceDiscoveryManager for a given XMPPConnection. This
	 * means that the service manager will respond to any service discovery
	 * request that the connection may receive.
	 * 
	 * @param connection
			...
			public void processPacket(Packet packet)
				...
				if (discoverItems != null
					...
					if (nodeInformationProvider != null) {
						...
						response.addItems(nodeInformationProvider
								.getNodeItems());
						// Add packet extensions
						response.addExtensions(nodeInformationProvider
		...
		packetListener = new PacketListener() {
			public void processPacket(Packet packet)
				...
				if (discoverInfo != null
					...
					// null
					// and if the request was not send to a node. If Entity Caps
					// are
					// enabled the client's identity and features are may also
					// added
					...
					if (discoverInfo.getNode() == null) {
						...
						if (nodeInformationProvider != null) {
							...
							response.addFeatures(nodeInformationProvider
									.getNodeFeatures());
							// Add node identities
							response.addIdentities(nodeInformationProvider
									.getNodeIdentities());
							// Add packet extensions
							response.addExtensions(nodeInformationProvider
									.getNodePacketExtensions());
	...
	/**
	 * Remove an identity from the client. Note that the client needs at least
	 * one identity, the default identity, which can not be removed.
	 * 
	 * @param identity
	 ...
	 *         XMPPConnection.
	 */
	public static synchronized ServiceDiscoveryManager getInstanceFor(
	 ...
	 * 
	 * @see <a href="http://xmpp.org/extensions/xep-0030.html#info-basic">XEP-30
	 *      Basic Protocol; Example 2</a>
	 *
	 * @param response
	...
	/**
	 * Returns the data form that is set as extended information for this
	 * Service Discovery instance (XEP-0128)
	 * 
	 * @see <a href="http://xmpp.org/extensions/xep-0128.html">XEP-128: Service
	 *      Discovery Extensions</a>
	...
	/**
	 * Returns the data form as List of PacketExtensions, or null if no data
	 * form is set. This representation is needed by some classes (e.g.
	 * EntityCapsManager, NodeInformationProvider)
	 * 
	 ...
	 * its JID. Use null as entityID to query the server
	 * 
	 * @param entityID
	...
	public DiscoverInfo discoverInfo(String entityID)
		...
		// returned
		EntityCapsManager.NodeVerHash nvh = EntityCapsManager
				.getNodeVerHashByJid(entityID);
	 ...
	 * @see <a href="http://xmpp.org/extensions/xep-0030.html#info-basic">XEP-30
	 *      Basic Protocol</a>
	 * @see <a href="http://xmpp.org/extensions/xep-0030.html#info-nodes">XEP-30
	 *      Info Nodes</a>
	 * 
	 * @param entityID
	...
	public DiscoverInfo discoverInfo(String entityID, String node)
		...
		Packet result = connection().createPacketCollectorAndSend(disco)
				.nextResultOrThrow();
	 ...
	 * @throws NotConnectedException
	 */
	public boolean canPublishItems(String entityID) throws NoResponseException,
	...
	public void publishItems(String entityID, String node,
			DiscoverItems discoverItems) throws NoResponseException,
			XMPPErrorException, NotConnectedException {
		discoverItems.setType(IQ.Type.set);
		discoverItems.setTo(entityID);
		discoverItems.setNode(node);

		connection().createPacketCollectorAndSend(discoverItems)
				.nextResultOrThrow();
	}

	/**
	 * Queries the remote entity for it's features and returns true if the given
	 * feature is found.
	 *
	 * @param jid
	...
	/**
	 * Create a cache to hold the 25 most recently lookup services for a given
	 * feature for a period of 24 hours.
	 */
	private Cache<String, List<String>> services = new Cache<String, List<String>>(
			25, 24 * 60 * 60 * 1000);
	 ...
	 * @param stopOnFirst
	 *            if true, stop searching after the first service was found
	 * @param useCache
	 *            if true, query a cache first to avoid network I/O
	 * @return a possible empty list of services providing the given feature
	 * @throws NoResponseException
	 * @throws XMPPErrorException
	 * @throws NotConnectedException
	 */
	public List<String> findServices(String feature, boolean stopOnFirst,
			boolean useCache) throws NoResponseException, XMPPErrorException,
			NotConnectedException {
		...
		try {
			...
			// Be extra robust here: Return the empty linked list and log this
			// situation
			LOGGER.log(Level.WARNING,
					"Could not discover information about service", e);
			return serviceAddresses;
		}
		...
		try {
			...
			LOGGER.log(Level.WARNING, "Could not discover items about service",
					e);
			return serviceAddresses;
		}
		for (DiscoverItems.Item item : items.getItems()) {
			try {
				// TODO is it OK here in all cases to query without the node
				// attribute?
				// MultipleRecipientManager queried initially also with the node
				// attribute, but this
	...
	/**
	 * Loads the ServiceDiscoveryManager with an EntityCapsManger that speeds up
	 * certain lookups.
	 * 
	 * @param manager
	 */
