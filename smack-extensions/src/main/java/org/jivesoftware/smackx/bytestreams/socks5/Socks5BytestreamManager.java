...
public final class Socks5BytestreamManager implements BytestreamManager {
						...
						connection
								.addConnectionListener(new AbstractConnectionListener() {
									...
									public void reconnectionSuccessful() {
										// re-create the manager for this
										// connection
										Socks5BytestreamManager
												.getBytestreamManager(connection);
									}
	 ...
	 * Bytestream initiation requests and resetting its internal state, which
	 * includes removing this instance from the managers map.
	 * <p>
	 * To re-enable the SOCKS5 Bytestream feature invoke
	 * {@link #getBytestreamManager(XMPPConnection)}. Using the file transfer
	 * API will automatically re-enable the SOCKS5 Bytestream feature.
	 */
	 ...
	 * @throws SmackException
	 *             if the target does not support SOCKS5.
	 * @throws XMPPException
	 */
	public Socks5BytestreamSession establishSession(String targetJID,
			String sessionID) throws IOException, InterruptedException,
			NoResponseException, SmackException, XMPPException {

		XMPPErrorException discoveryException = null;
		// check if target supports SOCKS5 Bytestream
		if (!supportsSocks5(targetJID)) {
			throw new FeatureNotSupportedException("SOCKS5 Bytestream",
					targetJID);
		}
		...
		try {
			...
			Packet response = connection.createPacketCollectorAndSend(
					initiation).nextResultOrThrow(getTargetResponseTimeout());
	 ...
	 */
	private boolean supportsSocks5(String targetJID)
			throws NoResponseException, XMPPErrorException,
			NotConnectedException {
		return ServiceDiscoveryManager.getInstanceFor(connection)
				.supportsFeature(targetJID, Bytestream.NAMESPACE);
	}
	 ...
	 * @throws NotConnectedException
	 */
	private List<String> determineProxies() throws NoResponseException,
		...
		for (Item item : discoverItems.getItems()) {
			...
			for (Identity identity : proxyInfo.getIdentities()) {
				...
				/*
				 * server is not a SOCKS5 proxy, blacklist server to skip next
				 * time a Socks5 bytestream should be established
				 */
	...
	private List<StreamHost> determineStreamHostInfos(List<String> proxies) {
		...
		for (String proxy : proxies) {
			...
			try {
				Bytestream response = (Bytestream) connection
						.createPacketCollectorAndSend(streamHostRequest)
						.nextResultOrThrow();
